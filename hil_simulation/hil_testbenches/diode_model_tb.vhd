...
------------------------------------------------------------------------
    ...
    begin
        if rising_edge(simulator_clock) then
            ...
            input_voltage := (sin(realtime * 2.0*math_pi * 1000.0));
            voltage_over_inductor := 0.0;

            if input_voltage > capacitor_voltage + 0.3 then
                current := (input_voltage - capacitor_voltage - 0.3)*10.0;
                voltage_over_inductor := input_voltage - capacitor_voltage - 0.3;
            else
                current := 0.0;
            end if;
            ...
            inductor_current  := inductor_current + (voltage_over_inductor)*L_gain;
            capacitor_voltage := capacitor_voltage + (inductor_current - capacitor_voltage/10.0)*c_gain;
