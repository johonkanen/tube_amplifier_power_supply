...
entity boost_control is
    port (
        ...
        inductor_current      : in integer range -2**15 to 2**15-1;
        input_voltage         : in integer range -2**15 to 2**15-1;
        dc_link_voltage       : in integer range -2**15 to 2**15-1;
        boost_control_ready   : out boolean;
        duty_ratio            : out natural range 0 to 2**16-1
    );
    ...
    begin
        if rising_edge(core_clock) then
            ...
            if control_counter < 128e6/120e3 then
                ...
                control_counter <= 0;
                request_current_control(current_control, self.current_ref, inductor_current);
...
------------------------------------------------------------------------
    ...
    begin
        if rising_edge(core_clock) then
            ...
            if boost_control_ready then
                set_duty(boost_interface, duty_ratio);
            end if;
    ...
    port map (
        ...
        inductor_current      => get_measurement(boost_interface,inductor_current)*2**4,
